...
data "template_file" "certificates" {
    ...
    vars {
      ...
      worker0_dns = "${aws_instance.kubernetes_workers.0.private_dns}"
      worker1_dns = "${aws_instance.kubernetes_workers.1.private_dns}"
      worker2_dns = "${aws_instance.kubernetes_workers.2.private_dns}"
    }
}
